...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        # continuous output targets? yes --> regression problem = no logistic regression
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error_term = np.multiply(
            hidden_error.T, (hidden_outputs * (1 - hidden_outputs)))

        # Weight step (input to hidden)
        delta_weights_i_h += np.matmul(X[:, None], hidden_error_term)
        # Weight step (hidden to output)
        delta_weights_h_o += np.matmul(
            hidden_outputs[:, None], output_error_term[:, None])
